...
def get_vac(name_vac, name_area):
    id_area = 113 # id РФ
    if name_area:
...
def graph_region(mas_vac: pd.DataFrame):
    try:
        mas_region = []
        for index, row in mas_vac.iterrows():
            if st := row['Город']:
